...
BEGIN_MESSAGE_MAP(CLogViewApp, CWinAppEx)
...
CLogViewApp::CLogViewApp()
{
	...
	SetAppID(_T("Huaiyu.tools.LogView.1.0"));
...
BOOL CLogViewApp::InitInstance()
{
	...
	char sAddr[256] = {0};
	::GetPrivateProfileString(_T("Server"), _T("Addr"), _T("0.0.0.0"), sAddr, 256, sCfgFile);
	WORD wPort = ::GetPrivateProfileInt(_T("Server"), _T("Port"), 0, sCfgFile);

	CMainFrame *pFrame = (CMainFrame *)m_pMainWnd;
	CLogViewView *pView = (CLogViewView *)pFrame->GetActiveView();
	m_client.Init(sAddr, wPort);
	m_client.SetREOp(pView);
	m_client.Start();
...
// CLogViewApp 自定义加载/保存方法

void CLogViewApp::PreLoadState()
{
	BOOL bNameValid;
	CString strName;
...
void CLogViewApp::LoadCustomState()
{
}

void CLogViewApp::SaveCustomState()
{
}
